...
class SubtractiveClustering:
    ...
    def __init__(self, r_1:float, r_2:float, eps = 0.001, verbose = False) -> None:
        '''
        ...
        self.r_1 = r_1; self.r_2 = r_2; self.eps = eps; self.verbose = verbose    
    ...
    def fit(self, data: npt.NDArray):
        ...
        max = numpy.max(self.potential)
        max_star = 0
        count = 1
        while(max >= self.eps * max_star):
            
            center = data[numpy.argmax(self.potential )]
            self.centers = numpy.vstack((self.centers, center))
            # Step 3: Update the Mountain Function
            self.potential  = self.update_mountain_function(self.potential , max, center, data)
            ...
            if self.verbose:
                print(f'SC iteration {count}, potential: {self.potential}')
            count += 1
        return self
